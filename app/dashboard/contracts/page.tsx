...
export default async function ContractsPage() {
  ...
  const { data: contracts } = await supabase
    .from('contracts')
    .select('id, title, status, created_at, signed_at, clients(name)')
    .eq('user_id', user!.id)
    ...
    <PageContent>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold tracking-tight text-gray-900">Contracts</h1>
          <Link href="/dashboard/contracts/new" className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 transition-colors">
            New contract
          </Link>
        </div>
        ...
        {!contracts?.length ? (
          <div className="rounded-2xl border border-dashed border-gray-200 p-12 text-center">
            ...
            <p className="mt-1 text-xs text-gray-400">Generate one from an accepted proposal, or start fresh.</p>
            <Link href="/dashboard/contracts/new" className="mt-3 inline-block text-sm font-medium text-gray-900 hover:underline">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {contracts.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 group transition-colors">
                      <td className="px-5 py-3.5">
                        <Link href={`/dashboard/contracts/${c.id}`} className="font-medium text-gray-900 hover:underline">
                      ...
                      <td className="px-4 py-3.5 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                        <DeleteButton endpoint={`/api/contracts/${c.id}`} />
                      </td>
                    </tr>
                  )
                })}
