...
export function ClientNotes({ clientId, initialNotes }: { clientId: string; initialNotes: Note[] }) {
  ...
  async function handleCatchUp() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        setCatchUpError(data.error === 'NO_NOTES'
      ...
      setCatchUpError('Could not generate summary. Try again.')
    } finally {
      setCatchingUp(false)
    }
  }
    ...
    <div className="space-y-4">
      ...
      {notes.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 bg-white p-8 text-center">
          <p className="text-sm text-gray-400">No notes yet. Add anything worth remembering about this client.</p>
        </div>
      ) : (
